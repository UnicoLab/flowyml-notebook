...
# %% [code] id=42486a9b "data_generation"
...
df = pd.DataFrame(
    {
        ...
        "created_at": [
            datetime.now() - timedelta(hours=np.random.randint(1, 720)) for _ in range(n)
        ],
    }
)
...
# %% [code] id=8dfbe29a "exploration"
# Filter and explore: high-performance experiments
top_experiments = df[(df["accuracy"] > 0.90) & (df["status"] == "completed")].sort_values(
