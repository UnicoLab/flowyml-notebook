...
def generate_report(
    ...
    save_path = output_path or os.path.join(
        tempfile.gettempdir(), f"{notebook.metadata.name}_report.{format}"
    )
    ...
    if format == "html":
        ...
        content = _generate_presentation_html(notebook, report_title, include_code)
        if not save_path.endswith(".html"):
            save_path = save_path.rsplit(".", 1)[0] + ".html"
    else:
        raise ValueError(f"Unsupported format: {format}. Use 'html', 'pdf', or 'presentation'.")
...
def _generate_html_report(
    ...
    for cell in notebook.cells:
        if cell.cell_type == CellType.MARKDOWN:
            ...
            # Outputs - skip raw text when rich outputs exist
            rich_types = {
                "dataframe",
                "chart",
                "image",
...
<body>
    <div class="report-container">
        ...
        <footer class="report-footer">
            Generated by FlowyML Notebook v{__version__} • {_escape_html(notebook.metadata.name)}
        </footer>
    </div>
</body>
</html>"""
...
def _generate_presentation_html(
    ...
    """Generate a presentation-style HTML with scroll-snap slides."""
    slides_html = []
    slide_num = 0
    ...
    slides_html.append(
        ...
        f'<p class="subtitle">{_escape_html(notebook.metadata.name)}</p>'
        f'<p class="date">{timestamp}</p>'
        f"</section>"
    )
    slide_num += 1
    ...
    for cell in notebook.cells:
        if cell.cell_type == CellType.MARKDOWN:
            slides_html.append(
                ...
                f'<div class="slide-content">{_md_to_html(cell.source)}</div>'
                f"</section>"
            )
            slide_num += 1
        elif cell.cell_type == CellType.CODE:
            ...
            if include_code:
                parts.append(
                    f'<div class="code-block"><pre><code>{_escape_html(cell.source)}</code></pre></div>'
                )
            for output in cell.outputs:
                parts.append(_render_output_html(output))
            if parts:
                slides_html.append(
                    f'<section class="slide">'
                    f'<div class="slide-content">{"".join(parts)}</div>'
                    f"</section>"
                )
                slide_num += 1
        elif cell.cell_type == CellType.SQL:
            ...
            if include_code:
                parts.append(
                    f'<div class="code-block"><pre><code class="sql">{_escape_html(cell.source)}</code></pre></div>'
                )
            for output in cell.outputs:
                parts.append(_render_output_html(output))
            if parts:
    ...
    slides_html.append(
        f'<section class="slide title-slide">'
        f"<h1>Thank you</h1>"
        f'<p class="subtitle">Generated by FlowyML Notebook v{__version__}</p>'
        f"</section>"
    )
    slide_num += 1

    body = "\n".join(slides_html)
    nav_dots = "".join(f'<span class="dot" data-slide="{i}"></span>' for i in range(slide_num))
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .slide-content h1 {{ font-size: 2rem; color: #3b82f6; }}
        .slide-content h2 {{ font-size: 1.5rem; color: #8b5cf6; }}
        ...
        .output-image img {{ max-width: 100%; border-radius: 8px; }}
...
<body>
    {body}
    <nav class="nav-dots">{nav_dots}</nav>
...
def _render_output_html(output: Any) -> str:
    ...
    otype = (
        output.output_type if hasattr(output, "output_type") else output.get("output_type", "text")
    )
...
def _dataframe_to_html(data: Any, metadata: dict | None = None) -> str:
    ...
    if numeric_cols:
        parts.append(
            '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px;padding:12px">'
        )
    ...
    if cat_cols:
        parts.append(
            '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;padding:0 12px 12px">'
        )
        for col in cat_cols:
            s = stats_map[col]
    ...
    for i, row in enumerate(data[:100]):
        cells = [
            f'<td style="text-align:center;color:#64748b;font-size:0.65rem;width:40px">{i}</td>'
        ]
        for col in columns:
            v = row.get(col, "")
            ...
            if v is None:
                ...
                cells.append(
                    f'<td style="text-align:right;color:#22d3ee">{_escape_html(str(v))}</td>'
                )
            else:
