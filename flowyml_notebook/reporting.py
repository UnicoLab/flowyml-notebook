...
def generate_report(
    ...
    save_path = output_path or os.path.join(tempfile.gettempdir(), f"{notebook.metadata.name}_report.{format}")
...
def _generate_html_report(
    ...
    for cell in notebook.cells:
        if cell.cell_type == CellType.MARKDOWN:
            ...
            rich_types = {'dataframe', 'chart', 'image', 'widget', 'pipeline_result', 'model_info', 'html'}
            has_rich = any(getattr(o, 'output_type', o.get('output_type', '')) in rich_types for o in cell.outputs)
...
def _generate_presentation_html(
    ...
    body = "\n".join(slides_html)
    nav_dots = "".join(
        f'<span class="dot" data-slide="{i}"></span>' for i in range(slide_num)
    )
