...
test(
    "simple assignment",
    lambda: (
        (_r := analyze_cell_dependencies("x = 42")[0]),
...
def t_cell():
    c = Cell()
    assert c.cell_type == CellType.CODE and len(c.id) == 8
...
def t_meta():
    nb = NotebookFile(
        metadata=NotebookMetadata(name="m", server="https://s.com", author="A", tags=["a", "b"])
    )
...
def t_pct():
    ...
    markers = [line for line in serialize_notebook(nb).split("\n") if line.startswith("# %%")]
